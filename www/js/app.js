...
lab2.factory('processMove', function() {
  return function() {
    self.checkWins =  function(player1_choice, player2_choice) {
      ...
      var aChoice = self.getChoiceAsNumber(a);
      var bChoice = self.getChoiceAsNumber(b);

      if (aChoice === bChoice) {
        return 0;
      }

      else if (aChoice - bChoice === 1 || bChoice - aChoice === 2) {
        return 1;
      }
      else if (bChoice - aChoice === 1 || aChoice - bChoice === 2) {
        return 2;
      }
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    $scope.wins = $scope.processMove.checkWins($stateParams.player1_choice, choice);
